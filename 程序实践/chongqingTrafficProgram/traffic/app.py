...
file_handler = RotatingFileHandler('logs/transit.log', maxBytes=10*1024*1024, backupCount=5, encoding='utf-8')
...
class TransitGraph:
    ...
    def clear(self):
        self.stations = {}  # 存储站点信息 {station_id: {'name': name, 'location': (lat, lng)}}
        self.routes = defaultdict(list)  # 存储路线信息 {station_id: [(next_station_id, line_number, distance)]}
        self.lines = defaultdict(set)  # 存储线路信息 {line_number: set(station_ids)}
        logging.info("清空图数据")
    ...
    def add_connection(self, station1_id, station2_id, line_number):
        try:
            ...
            logging.info(f"添加连接成功: {station1_id}({self.stations[station1_id]['name']}) <-> {station2_id}({self.stations[station2_id]['name']}), 线路={line_number}, 距离={distance:.2f}米")
        except Exception as e:
    ...
    def get_debug_info(self):
        return {
            ...
            'routes': {k: [(next_id, line, f"{dist:.2f}m") for next_id, line, dist in v] for k, v in self.routes.items()}
        }
...
def find_shortest_path(graph, start_id, end_id):
    try:
        ...
        logging.debug(f"图中共有 {len(graph.stations)} 个站点")
        ...
        distances = {station_id: float('infinity') for station_id in graph.stations}
        distances[start_id] = 0
        previous = {station_id: None for station_id in graph.stations}
        pq = [(0, start_id)]
        path_lines = {station_id: [] for station_id in graph.stations}
        
        logging.debug(f"初始化完成，开始寻路")
        ...
        while pq:
            ...
            # 检查当前站点是否有相邻站点
            if current_station not in graph.routes:
                logging.debug(f"站点 {current_station} 没有相邻站点")
                continue
                
            for next_station, line_number, distance in graph.routes[current_station]:
                if next_station in visited:
                    continue
                    
                new_distance = distances[current_station] + distance
                
                logging.debug(f"检查相邻站点: {next_station}, 线路: {line_number}, 新距离: {new_distance}, 原距离: {distances[next_station]}")
...
def search():
    ...
    # 调用高德地图POI搜索API
    url = f'https://restapi.amap.com/v3/place/text?key={AMAP_KEY}&keywords={keyword}&city={city}&offset=10&page=1&extensions=all'
...
def get_route():
    try:
        ...
        dest_location = get_location(destination)
        
        if not origin_location or not dest_location:
            return jsonify({'status': '0', 'info': '位置信息格式错误'})
        
        logging.info(f"位置解析成功: 起点={json.dumps(origin_location)}, 终点={json.dumps(dest_location)}")
        
        # 获取附近的公交站点
        def get_nearby_stations(location, location_type="起点"):
            try:
                url = f'https://restapi.amap.com/v3/place/around?key={AMAP_KEY}&location={location["location"][1]},{location["location"][0]}&keywords=公交站&types=150700&radius=1000&extensions=all'
                response = requests.get(url)
                data = response.json()
                
                if data.get('status') != '1':
                    logging.error(f"获取{location_type}附近站点失败: {data.get('info')}")
                    return []
        ...
        origin_stations = get_nearby_stations(origin_location, "起点")
        dest_stations = get_nearby_stations(dest_location, "终点")
        
        if not origin_stations or not dest_stations:
            return jsonify({'status': '0', 'info': '未找到附近站点'})
        
        # 更新图数据结构
        for station in origin_stations + dest_stations:
            station_id = station['id']
            station_name = station['name']
            station_location = tuple(map(float, station['location'].split(',')))[::-1]
            transit_graph.add_station(station_id, station_name, station_location)
        
        # 获取公交线路信息
        url = f'https://restapi.amap.com/v3/direction/transit/integrated?key={AMAP_KEY}&origin={origin}&destination={destination}&city=重庆&strategy={strategy}&extensions=all'
        ...
        if transit_data.get('status') == '1' and transit_data.get('route'):
            ...
            # 直接连接起点附近站点
            for i in range(len(origin_stations)):
                for j in range(i + 1, len(origin_stations)):
                    station1 = origin_stations[i]
                    station2 = origin_stations[j]
                    transit_graph.add_connection(station1['id'], station2['id'], 'nearby_connection')
            
            # 直接连接终点附近站点
            for i in range(len(dest_stations)):
                for j in range(i + 1, len(dest_stations)):
                    station1 = dest_stations[i]
                    station2 = dest_stations[j]
                    transit_graph.add_connection(station1['id'], station2['id'], 'nearby_connection')
            ...
            for i, transit in enumerate(transits):
                ...
                for segment in segments:
                    ...
                    if isinstance(walking, dict) and 'steps' in walking:
                        steps = walking['steps']
                    elif isinstance(walking, list):
                        steps = walking
                    else:
                        steps = []
                    ...
                    if isinstance(walking, dict):
                        walk_origin = walking.get('origin')
                        walk_dest = walking.get('destination')
                    
                    if walk_origin and walk_dest:
                        walk_origin_loc = tuple(map(float, walk_origin.split(',')))[::-1]
                        walk_dest_loc = tuple(map(float, walk_dest.split(',')))[::-1]
                        
                        # 为步行段创建虚拟站点
                        walk_origin_id = f"walk_{walk_origin}"
                        walk_dest_id = f"walk_{walk_dest}"
                        
                        transit_graph.add_station(walk_origin_id, "步行起点", walk_origin_loc)
                        transit_graph.add_station(walk_dest_id, "步行终点", walk_dest_loc)
                        
                        # 连接步行段的起点和终点
                        distance = calculate_distance(walk_origin_loc, walk_dest_loc)
                        transit_graph.add_connection(walk_origin_id, walk_dest_id, 'walking')
                        logging.info(f"添加步行连接: {walk_origin_id} -> {walk_dest_id}, 距离={distance:.2f}米")
                        
                        # 连接步行段与附近的公交站点
                        for station_id, station_info in transit_graph.stations.items():
                            if 'walk_' not in station_id:  # 只考虑实际的公交站点
                                station_loc = station_info['location']
                                
                                # 检查与步行起点的距离
                                origin_distance = calculate_distance(walk_origin_loc, station_loc)
                                if origin_distance <= 100:  # 100米以内的站点
                                    transit_graph.add_connection(station_id, walk_origin_id, 'walking_connection')
                                    logging.info(f"添加步行起点连接: {station_id} -> {walk_origin_id}, 距离={origin_distance:.2f}米")
                                
                                # 检查与步行终点的距离
                                dest_distance = calculate_distance(walk_dest_loc, station_loc)
                                if dest_distance <= 100:  # 100米以内的站点
                                    transit_graph.add_connection(walk_dest_id, station_id, 'walking_connection')
                    ...
                    # 处理公交段
                    bus = segment.get('bus', {})
                    if isinstance(bus, dict) and 'buslines' in bus:
                    ...
                    # 记录地铁换乘站点
                    metro_stations = {}  # {station_name: {line_id: station_id}}
                    ...
                    for busline in buslines:
                        try:
                            ...
                            for stop in all_stops:
                                ...
                                stop_id = stop.get('id')
                                if not stop_id:
                                    stop_id = f"stop_{stop.get('name')}_{stop.get('location')}"
                                stop_name = stop.get('name', '未知站点')
                                location = stop['location'].split(',')
                                stop_location = (float(location[1]), float(location[0]))  # 转换为(lat, lng)格式
                                
                                # 添加站点
                                transit_graph.add_station(stop_id, stop_name, stop_location)
                                ...
                                # 检查是否是换乘站点（通过名称和位置判断）
                                for existing_id, existing_info in transit_graph.stations.items():
                                    if existing_id != stop_id:
                                        # 计算两个站点的距离
                                        distance = calculate_distance(stop_location, existing_info['location'])
                                        # 如果站点名称相同或距离很近，认为是换乘站点
                                        if existing_info['name'] == stop_name or distance <= 100:
                                            transit_graph.add_connection(stop_id, existing_id, 'transfer')
                                            logging.info(f"添加换乘连接: {stop_id}({stop_name}) <-> {existing_id}({existing_info['name']})")
                                
                                # 如果有上一个步行终点，检查是否需要连接
                                if last_station_id and 'walk_' in last_station_id:
                                    last_location = transit_graph.stations[last_station_id]['location']
                                    distance = calculate_distance(stop_location, last_location)
                                    if distance <= 100:  # 100米以内的站点
                                        transit_graph.add_connection(last_station_id, stop_id, 'walking_connection')
                            ...
                            for i in range(len(all_stops) - 1):
                                ...
                                # 使用完整的站点ID
                                current_id = current_stop.get('id')
                                if not current_id:
                                    current_id = f"stop_{current_stop.get('name')}_{current_stop.get('location')}"
                                    
                                next_id = next_stop.get('id')
                                if not next_id:
                                    next_id = f"stop_{next_stop.get('name')}_{next_stop.get('location')}"
                                
                                if current_id in transit_graph.stations and next_id in transit_graph.stations:
                                    # 计算实际距离
                                    current_location = tuple(map(float, current_stop['location'].split(',')))[::-1]
                                    next_location = tuple(map(float, next_stop['location'].split(',')))[::-1]
                                    distance = calculate_distance(current_location, next_location)
                                    
                                    transit_graph.add_connection(
                                        current_id,
                                        next_id,
                                        line_name
                                    )
                            ...
                            # 更新最后访问的站点为当前线路的终点站
                            if arrival_stop:
                                last_station_id = arrival_stop.get('id')
                                if not last_station_id:
                                    last_station_id = f"stop_{arrival_stop.get('name')}_{arrival_stop.get('location')}"
                        ...
                        if len(line_stations) > 1:  # 如果一个站点出现在多条线路上
                            ...
                            for i in range(len(station_ids)):
                                for j in range(i + 1, len(station_ids)):
                                    transit_graph.add_connection(station_ids[i], station_ids[j], 'metro_transfer')
                                    logging.info(f"添加地铁换乘连接: {station_ids[i]} <-> {station_ids[j]}")
            
            # 输出连接信息
            logging.info("站点连接情况:")
            for station_id, routes in transit_graph.routes.items():
                station_name = transit_graph.stations[station_id]['name']
                connections = [f"{transit_graph.stations[next_id]['name']}(via {line})" for next_id, line, _ in routes]
                logging.info(f"{station_name} 连接到: {', '.join(connections)}")
                
            # 输出图的调试信息
            debug_info = transit_graph.get_debug_info()
            ...
            for start_station in origin_stations:
                for end_station in dest_stations:
                    try:
                        path, lines, distance = find_shortest_path(transit_graph, start_station['id'], end_station['id'])
                        if distance < min_distance:
                            ...
                            best_path = {
                                ...
                                'lines': lines,
                                'distance': distance,
                                'stations': [transit_graph.stations[station_id] for station_id in path]
                            }
...
def route_api():
    origin = request.args.get('origin')
    destination = request.args.get('destination')
    strategy = request.args.get('strategy', '0')  # 默认使用时间最少策略

    transit_url = f'https://restapi.amap.com/v3/direction/transit/integrated?key={AMAP_KEY}&origin={origin}&destination={destination}&city=重庆&strategy={strategy}&extensions=all'
    transit_response = requests.get(transit_url)
    driving_url = f'https://restapi.amap.com/v3/direction/driving?key={AMAP_KEY}&origin={origin}&destination={destination}&extensions=all'
